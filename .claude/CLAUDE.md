...
Guidance for Claude Code when working in this repository (a Flutter app: SonicRelay).

Before making changes, read and follow the repository-wide instructions in
`/AGENTS.md`, especially the semantic versioning policy and the changelog policy.
...
`CI/CD` owns it (see the `version` job in `.github/workflows/flutter-ci.yml`).
...
Include every locale the Play Store listing supports, using exactly these
Play Console tags and this order:

* `en-US`
* `pt-BR`

These tags correspond to the directories in `fastlane/metadata/android/`.
...
5. Do not translate locale tags or use unsupported regional variants (e.g.
   `en-GB`, `pt-PT`).
6. Preserve relevant emojis.
7. If a feature must remain hidden, such as an easter egg, use a subtle hint
   without revealing it.
8. When adding a new supported Play Store locale, mention it in every locale
   block and add the matching `fastlane/metadata/android/<locale>/` directory.
